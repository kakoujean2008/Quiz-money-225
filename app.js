...
function handleAnswer(key, btn, correct) {
  ...
  if (key === correct) {
    ...
    setTimeout(() => {
      endSoloGame(false, true); // wrongAnswer = true
    }, 1800);
  }
}
...
async function endSoloGame(quit = false, wrongAnswer = false, encaissed = false) {
  clearInterval(soloTimer);

  // Transfer remaining session gains only if game ended naturally (all 10 questions) — not on wrong answer/timeout
  ...
  // Réactiver le bouton quitter (partie terminée)
  const backBtn = document.querySelector("#solo-screen .btn-back");
  if (backBtn) {
  ...
  let emoji = "😅";
  if (encaissed) emoji = "🎉";
  else if (soloScore >= 8) emoji = "🏆";
  else if (soloScore >= 5) emoji = "🎉";
  else if (soloScore >= 3) emoji = "👍";

  document.getElementById("result-emoji").textContent = emoji;
  document.getElementById("result-title").textContent =
    encaissed ? "🎉 Félicitations !" :
    quit ? "Partie interrompue" :
    wrongAnswer ? "❌ Partie perdue !" : "Partie terminée !";
...
async function processRecharge() {
  ...
  document.getElementById("recharge-confirm-pseudo").textContent  = currentUser.pseudo;
  document.getElementById("recharge-confirm-phone").textContent   = currentUser.phone;
  document.getElementById("recharge-confirm-method").textContent  = selectedPaymentMethod;
  document.getElementById("recharge-confirm-amount").textContent  = amount.toLocaleString('fr-FR') + " FCFA";
  document.getElementById("modal-recharge-confirm").classList.remove("hidden");
}

function confirmRechargeAndPay() {
  document.getElementById("modal-recharge-confirm").classList.add("hidden");
